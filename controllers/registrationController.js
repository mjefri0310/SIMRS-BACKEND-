...
// const generateNumber = require("../utils/generateRegistrationNumber");
...
exports.index = async (req, res) => {
  try {
    ...
    const registrations = await Registration.find(filter)
      ...
      .populate(
        "patient_id",
        "name medical_record_number nik gender phone address birth_date",
      )
...
exports.store = async (req, res) => {
  try {
    ...
    const queueNumber = await generateQueueNumber(req.body.clinic_id);
    const queueCode = `A${String(queueNumber).padStart(3, "0")}`;
