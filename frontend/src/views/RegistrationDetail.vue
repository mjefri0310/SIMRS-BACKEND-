...
import { getPatientById, getLastRegistration } from "../services/patientService";
import { getAll as getInsurances } from "../services/insuranceService";
import { getAll as getDoctors } from "../services/doctorService";
import { getAll as getPolikliniks } from "../services/clinicService";
...
const loadPatient = async (id) => {
  try {
    ...
    const [poliklinikRes, doctorRes, serviceTypeRes, insuranceRes] =
      await Promise.all([
        getPolikliniks(),
        getDoctors(),
        getServiceTypes(),
        ...
        <template #content>
          <div class="grid">
            <div class="col-3 mb-3">
              ...
              <div class="font-medium">{{ selectedPatient?.nik || "-" }}</div>
            </div>
            <div class="col-2 mb-3">
              <div class="text-500 text-sm">Gender</div>
              <div class="font-medium">{{ selectedPatient?.gender || "-" }}</div>
            </div>
            <div class="col-2 mb-3">
              <div class="text-500 text-sm">Tanggal Lahir</div>
              <div class="font-medium">
                {{ selectedPatient?.birth_date ? new Date(selectedPatient.birth_date).toLocaleDateString() : "-" }}
              </div>
            </div>
            <div class="col-2 mb-3">
            ...
            <div class="col-3">
              ...
              <div class="font-medium line-height-3">{{ selectedPatient?.address || "-" }}</div>
            </div>
          </div>
        </template>
      </Card>
        ...
        <template #content>
          <div class="grid">
            ...
            <div class="col-12 mb-3">
              ...
              <div class="font-medium">{{ lastRegistration.doctor_id?.name || "-" }}</div>
            </div>
            <div class="col-12 mb-3">
              <div class="text-500 text-sm">Keluhan</div>
              <div class="font-medium line-height-3">{{ lastRegistration.complaint || "-" }}</div>
            </div>
          </div>
        </template>
      </Card>
